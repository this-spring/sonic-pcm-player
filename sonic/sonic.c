...
int sonicReadCharFromStream(sonicStream stream, unsigned char* samples, int maxSamples)
{
    return sonicReadFloatFromStream(stream, (float*)samples, maxSamples);
}
...
int sonicReadFloatFromStream(sonicStream stream, float* samples,
  ...
  buffer = stream->outputBuffer;
    ...
    // printf("sonicReadFloatFromStream buffer[i]:%d\n", buffer[i]);
  // } 
  count = numSamples * stream->numChannels;
  // printf("sonicReadFloatFromStream numSamples:%d stream->numChannels:%d count:%d\n", numSamples, stream->numChannels, count);
  while (count--) {
    //*samples++ = (*buffer++) / 32767.0f;
    *samples = (float)(*buffer) / 32767.0f;
    // if (count < 5) {
    //   printf("starhan ---count %d ---%f  %d %f- \n", count, *samples, *buffer, (float)(*buffer));
    // }
  ...
  /*
  for (int i = 0; i < 10; i += 1) {
    printf("sonicReadFloatFromStream samples[i]:%f\n", samples[i]);
  }
  */
  if (remainingSamples > 0) {
...
int sonicWriteCharToStream(sonicStream stream, unsigned char* samples, int numSamples)
{
    return sonicWriteFloatToStream(stream, (float*)samples, numSamples);
}
